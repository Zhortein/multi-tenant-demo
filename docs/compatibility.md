...
The demo requires the exact `1.0.0-rc.5` bundle release candidate and `composer.lock` records the tagged source commit. The committed lock file is the reproducibility boundary: ordinary installations must run `composer install` and must not resolve a development branch or a future stable version implicitly.
...
# RC5 validation target

The fail-closed consumer target is PHP 8.5.9, Symfony 8.1.5, Doctrine ORM 3.6.8, DBAL 4.4.4, DoctrineBundle 3.3.1, DoctrineMigrationsBundle 4.0.1, and PostgreSQL 16 through 18. The current published demo graph remains Symfony 7.4 until its separately reviewed Symfony 8.1 migration is complete. The exact RC5 release is installed from Packagist and recorded in `composer.lock`; machine-specific Composer repositories must never be committed.
